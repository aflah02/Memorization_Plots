...
st.set_page_config(page_title="Accuracy Plots for PEFT Methods", page_icon="1️⃣")
...
for method, suffix in zip(chosen_method, suffixes_for_chosen_methods):
    for string_type in chosen_type:
        for string_len in chosen_string_len:
            ...
            # remove .png from end
            title = title[:-len('.png')]
            # remove Epochs- 100 BSize- 1 Seed- 42 from middle
            title = title.replace(' Epochs- 100 BSize- 1 Seed- 42', '')
            st.markdown('#### ' + title)
